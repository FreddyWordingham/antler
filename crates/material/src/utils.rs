...
#[inline]
pub fn reflect(incident: Vector3<f32>, normal: Vector3<f32>) -> Unit<Vector3<f32>> {
    Unit::new_normalize(incident - 2.0 * incident.dot(&normal) * normal)
}

#[inline]
pub fn refract(incident: Vector3<f32>, normal: Vector3<f32>, eta: f32, cos_theta: f32) -> Unit<Vector3<f32>> {
